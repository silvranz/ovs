...
insert  into `generateweblog`(`GenerateWebID`,`StoreID`,`TemplateID`,`TemplateType`,`UserID`,`GenerateDate`,`Browser`,`BrowserVersion`,`IPAddress`,`Device`,`OS`,`CreatedUser`,`CreatedDate`,`AuditedActivity`,`AuditedUser`,`AuditedTime`) values (1,1,3,'CC1',1,'2016-08-24 14:29:12','Mozilla','5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/52.0.2743.116 Safari/537.','::1','pc','Win32',1,'2016-08-24 14:29:12','C',NULL,NULL);
...
insert  into `store`(`StoreID`,`StoreName`,`StoreDomain`,`StoreImage`,`StoreProjectName`,`UserID`,`TemplateID`,`StoreJson`,`CreatedUser`,`CreatedDate`,`AuditedActivity`,`AuditedUser`,`AuditedTime`) values (1,'thelana','thelana','thelana.jpg','thelana',1,3,NULL,1,'2016-08-24 14:29:12','C',NULL,NULL);
...
insert  into `template`(`TemplateID`,`TemplateTitle`,`TemplateImage`,`TemplateSubCategoryID`,`TemplateProjectName`,`TemplateJson`,`TemplateType`,`CreatedUser`,`CreatedDate`,`AuditedActivity`,`AuditedUser`,`AuditedTime`) values (1,'Company Profile Pink','asd.jpg',2,'template','satu','CC1',1,'2016-07-20 16:22:41','C',NULL,NULL),(2,'Monochrome','asd.jpg',3,'template','dua','CC1',1,'2016-07-20 16:23:15','C',NULL,NULL),(3,'Casual Much','asd.jpg',1,'template','tiga','CC1',1,'2016-07-20 16:23:49','C',NULL,NULL),(4,'Colourful Dress','asd.jpg',3,'template','empat','CC1',1,'2016-07-20 16:24:12','C',NULL,NULL);
...
/* Procedure structure for procedure `GetDomainName` */

/*!50003 DROP PROCEDURE IF EXISTS  `GetDomainName` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `GetDomainName`(
	IN ParamStoreID int
)
BEGIN
	SELECT StoreDomain from store
...
/* Procedure structure for procedure `GetTemplateType` */

/*!50003 DROP PROCEDURE IF EXISTS  `GetTemplateType` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `GetTemplateType`(
	IN ParamTemplateID INT,
	IN ParamStoreID INT
)
BEGIN
	IF ParamTemplateID is null
	THEN
		SET @TemplateID = (SELECT TemplateID from store where StoreID = ParamStoreID and AuditedActivity <> 'D' limit 1);
	ELSE 
		SET @TemplateID = ParamTemplateID;
	END IF;
	
	SET @TemplateType = (SELECT TemplateType from template where TemplateID = @TemplateID and AuditedActivity <> 'D' LIMiT 1);
	IF @TemplateType IS NOT NULL
	THEN
		SELECT @TemplateType 'TemplateType';
	ELSE
		SELECT -1 AS 'TemplateType';
	END IF;
    END */$$
DELIMITER ;
...
BEGIN
	SET @TemplateType = (SELECT TemplateType FROM template WHERE TemplateID = ParamTemplateID AND AuditedActivity <> 'D' LIMIT 1);
	...
	THEN
		...
		(StoreID, TemplateID, TemplateType, UserID, GenerateDate, Browser, BrowserVersion, IPAddress, Device, OS,
		CreatedUser, CreatedDate, AuditedActivity)
		VALUES
		(@StoreID, ParamTemplateID, @TemplateType, ParamUserID, CURRENT_TIMESTAMP(), ParamBrowser,
		ParamBrowserVersion, ParamIPAddress, ParamDevice, ParamOS, ParamUserID, CURRENT_TIMESTAMP(), 'C');
		SELECT @StoreID 'StoreID', @TemplateType 'TemplateType';
	ELSE
		SELECT -1 'StoreID', -1 AS 'TemplateType';
	END IF;
   END */$$
DELIMITER ;
