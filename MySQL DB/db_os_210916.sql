...
insert  into `generateweblog`(`GenerateWebID`,`StoreID`,`TemplateID`,`TemplateType`,`UserID`,`GenerateDate`,`Browser`,`BrowserVersion`,`IPAddress`,`Device`,`OS`,`CreatedUser`,`CreatedDate`,`AuditedActivity`,`AuditedUser`,`AuditedTime`) values (1,1,3,'CC1',7,'2016-09-06 18:45:55','Mozilla','5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/52.0.2743.116 Safari/537.','::1','pc','Win32',7,'2016-09-06 18:45:55','C',NULL,NULL);
...
insert  into `store`(`StoreID`,`StoreName`,`StoreDomain`,`StoreImage`,`StoreProjectName`,`UserID`,`TemplateID`,`StoreJson`,`CreatedUser`,`CreatedDate`,`AuditedActivity`,`AuditedUser`,`AuditedTime`) values (1,'mythelana','mythelana','mythelana.jpg','mythelana',7,3,NULL,7,'2016-09-06 18:45:55','C',NULL,NULL);
...
/* Procedure structure for procedure `DeactivateWebsite` */

/*!50003 DROP PROCEDURE IF EXISTS  `DeactivateWebsite` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `DeactivateWebsite`(
	in ParamUserID int,
	in ParamStoreID int
)
BEGIN
...
BEGIN
	...
	ORDER BY 
	UNIX_TIMESTAMP(CreatedDate) DESC
